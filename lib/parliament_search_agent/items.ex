defmodule ParliamentSearchAgent.Items do
  import Ecto.Query
  ...
  def upsert_broadcast(attrs) do
    ...
    if existing do
      ...
      |> case do
        {:ok, record} ->
          Phoenix.PubSub.broadcast(ParliamentSearchAgent.PubSub, "items:new", {:new_item, record})
          {:ok, :new, record}
  ...
  defp tap_broadcast_update({:ok, record} = result) do
    Phoenix.PubSub.broadcast(ParliamentSearchAgent.PubSub, "items:updated", {:updated_item, record})
    result
  end
