defmodule ParliamentSearchAgent.Workers.BackfillReportsWorker do
  ...
  def perform(%Oban.Job{args: %{"page" => page, "total" => total}}) do
    ...
    Logger.info("[backfill_reports] Fetching page #{page}")

    case ParlinfoClient.scrape(url) do
      {:ok, []} ->
        Logger.info("[backfill_reports] Done — #{total} total items across #{page} pages")
        :ok

      {:ok, items} ->
        new_count =
          Enum.count(items, fn item ->
            match?({:ok, :new, _}, Items.upsert_report(item))
          end)

        Logger.info("[backfill_reports] Page #{page}: #{length(items)} fetched, #{new_count} new")
      ...
      {:error, :waf_blocked} ->
        Logger.warning("[backfill_reports] WAF block on page #{page} — snoozing 10 minutes")
        {:snooze, 600}

      {:error, reason} ->
        Logger.error("[backfill_reports] Error on page #{page}: #{inspect(reason)}")
        {:error, reason}
    end
  end
end
